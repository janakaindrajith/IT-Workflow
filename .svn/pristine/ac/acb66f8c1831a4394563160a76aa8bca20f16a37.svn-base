...
namespace quickinfo_v2.Connectivity
{
    public static class DatabaseUtility
    {
        ...
        public static void closeConnection(OracleConnection _conn)
        {
            if (_conn.State == ConnectionState.Open)
                _conn.Close();
        }

        public static OracleConnection OpenConnectionDR()
        {
            OracleConnection _conn = new OracleConnection();

            _conn.ConnectionString = ConfigurationManager.ConnectionStrings["CGConnectionStringDR"].ConnectionString;
            _conn.Open();
            return _conn;
        }
